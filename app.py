...
st.set_page_config(page_title="Computer Visual",
                   page_icon="🧊")
lebar = 0
tinggi = 0
...
def drawBoundingBox(processed_image, box, treshold):
    ...
    mask = cv2.inRange(processed_image_hsv, np.array(
        [145, 32, 55]), np.array([217, 255, 255]))

    closing = cv2.morphologyEx(
        mask, cv2.MORPH_CLOSE, kernel=np.ones((11, 11), np.uint8))
    opening = cv2.morphologyEx(
        closing, cv2.MORPH_OPEN, kernel=np.ones((7, 7), np.uint8))
    ...
    for i in range(0, tresh):
        ...
        start_point = [random.randint(
            0, lebar_imaage), random.randint(0, tinggi_imaage)]
        image = cv2.rectangle(image, (start_point[0], start_point[1]), (start_point[1] + boxes, start_point[1] + boxes),
                              color[x], 10)
        image = cv2.putText(image, label[x], (start_point[0], start_point[1]), cv2.FONT_HERSHEY_SIMPLEX, 0.9, color[x],
                            2)
...
def main_loop():
    hide_st_style = """
                ...
                footer:after {content : "Mastri"; color: red}

                header {visibility: hidden;}
                </style>
                <h1>Deteksi Plasmodium Berghei</h1>
                """
    ...
    rbc = st.sidebar.checkbox('Sel Darah Merah')
    inf = st.sidebar.checkbox('Infeksi')
    etc = st.sidebar.checkbox('Lain - lain')
    blur_rate = st.sidebar.slider(
        "Blur  Gambar/Citra", min_value=0.5, max_value=3.5)
    brightness_amount = st.sidebar.slider(
        "Kecerahan  Gambar/Citra", min_value=-50, max_value=50, value=0)
    apply_enhancement_filter = st.sidebar.checkbox('Lebih Detail Gambar/Citra')
    st.sidebar.text("Hasil Deteksi Parameter")
    bounding_box = st.sidebar.slider(
        "Bounding Box", min_value=0.0, max_value=1.0, value=0.5)
    ...
    #
    original_image = brighten_image(original_image, brightness_amount)
    original_image = blur_image(original_image, blur_rate)
    if (apply_enhancement_filter):
        original_image = enhance_details(original_image)
    # print("apply_enhancement_filter = " + str(apply_enhancement_filter))

    original_image = cv2.resize(original_image, None, fx=0.4, fy=0.4)
    height, width, channels = original_image.shape

    blob = cv2.dnn.blobFromImage(
        original_image, 0.00392, (416, 416), (0, 0, 0), True, crop=False)
    ...
    for i in range(len(boxes)):
        if i in indexes:
            ...
            if label == "infected":  # person
                print(label)
                sInfected = sInfected + 1
                x_inf, y_inf, w_inf, h_inf = boxes[i]
                color = colors[class_ids[i]]
                # cv2.putText(processed_image, label, (x_inf, y_inf + 30), font, 3, color, 3)

                # line/font thickness
                tl = round(
                    0.002 * (processed_image.shape[0] + processed_image.shape[1]) / 2) + 1
                c1, c2 = (int(x_inf), int(y_inf)
                          ), (int(w_inf), int(h_inf))
                if label and inf:
                    cv2.rectangle(processed_image, (x_inf, y_inf), (x_inf + w_inf, y_inf + w_inf), color,
                                  2)
                    tf = max(tl - 1, 1)  # font thickness
                    t_size = cv2.getTextSize(
                        label, 0, fontScale=tl / 3, thickness=tf)[0]
                    c2 = c1[0] + t_size[0], c1[1] - t_size[1] - 3
                    cv2.rectangle(processed_image, c1, c2,
                                  color, -1, cv2.LINE_AA)  # filled
                    cv2.putText(processed_image, label + " " + str(int(confidences[i] * 100)) + "%", (c1[0], c1[1] - 2),
                                0,
                                tl / 3, [225, 255, 255], thickness=tf, lineType=cv2.LINE_AA)
                    cv2.circle(processed_image, (int(x_inf + int(w_inf / 2)), int(y_inf + int(h_inf / 2))),
                               4, color,
                               -1)
                    cv2.putText(processed_image,
                                str(int(x_inf + int(w_inf / 2))) +
                                ", " + str(int(y_inf + int(h_inf / 2))),
                                (int(x_inf + int(w_inf / 2) + 10),
                                 int(y_inf + int(h_inf / 2) + 10)), font,
                                tl / 2, [255, 255, 255], thickness=tf, lineType=cv2.LINE_AA)
            # =====
            if label == "rbc":  # person
                sRedBloodCells = sRedBloodCells + 1
                ...
                tl = round(
                    0.002 * (processed_image.shape[0] + processed_image.shape[1]) / 2) + 1
                c1, c2 = (int(x_person), int(y_person)
                          ), (int(w_person), int(h_person))
                if label and rbc:
                    cv2.rectangle(processed_image, (x_person, y_person), (x_person + w_person, y_person + w_person), color,
                                  2)
                    tf = max(tl - 1, 1)  # font thickness
                    t_size = cv2.getTextSize(
                        label, 0, fontScale=tl / 3, thickness=tf)[0]
                    ...
                    cv2.circle(processed_image, (int(x_person + int(w_person / 2)), int(y_person + int(h_person / 2))), 4, color,
                               -1)
                    cv2.putText(processed_image,
                                str(int(x_person + int(w_person / 2))) + ", " +
                                str(int(y_person + int(h_person / 2))),
                                (int(x_person + int(w_person / 2) + 10),
                                 int(y_person + int(h_person / 2) + 10)), font,
                                tl / 2, [255, 255, 255], thickness=tf, lineType=cv2.LINE_AA)
            # =====
            if label == "other":  # person
                ...
                if label and etc:
                    ...
                    cv2.putText(processed_image, label + " " + str(int(confidences[i] * 100)) + "%", (c1[0], c1[1] - 2),
                                0,
                                tl / 3, [225, 255, 255], thickness=tf, lineType=cv2.LINE_AA)
                    cv2.circle(processed_image, (int(x_person + int(w_person / 2)), int(y_person + int(h_person / 2))),
                               4, color,
                               -1)
                    cv2.putText(processed_image,
                                str(int(x_person + int(w_person / 2))) +
    ...
    st.text("Infeksi : " + str(sInfected))
    st.text("Sel Darah Merah : " + str(sRedBloodCells))
    val = 0
    if (sInfected != 0 and sRedBloodCells != 0):
        val = ((sInfected / sRedBloodCells) * 100)
        if val >= 100:
            val = 1
    st.text("Parasitemia (%) : " +
            str(format(val, ".2f")))
